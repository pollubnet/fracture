...
public class PrefixesGenerator
{
    ...
    {
        var maxAbsoluteStatValue = -1;
        var maxAbsoluteStatName = ItemStat.Luck;

        foreach (var stat in Enum.GetValues<ItemStat>())
        {
            var absoluteStatValue = Math.Abs(item.Statistics.GetStatFromItemStat(stat));
    ...
    {
        var statValue = item.Statistics.GetStatFromItemStat(itemStat);
    ...
    {
        var totalStats = Enum.GetValues<ItemStat>()
            .Aggregate(0, (acc, y) => acc + item.Statistics.GetStatFromItemStat(y));
        var statsNumber = Enum.GetNames(typeof(ItemStat)).Length;
        var statsAverage = (float)totalStats / statsNumber;
        ...
        var stat = FindMaxAbsoluteStat(item);
